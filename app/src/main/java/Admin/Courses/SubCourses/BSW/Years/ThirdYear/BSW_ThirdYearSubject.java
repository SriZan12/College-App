...
import Admin.Courses.SubCourses.BSW.Years.SecondYear.BswSecondYearSubjectAdapter;


public class BSW_ThirdYearSubject extends Fragment {

    FragmentBSWThirdYearSubjectBinding subjectBinding;
    BSW_ThirdYearSubjectAdapter subjectAdapter;
    String Role;

    public BSW_ThirdYearSubject(String role) {
        this.Role = role;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        subjectBinding =  FragmentBSWThirdYearSubjectBinding.inflate(inflater, container, false);

        subjectBinding.bswThirdSubRecycler.setLayoutManager(new LinearLayoutManager(getContext()));
        ...
        FirebaseRecyclerOptions<SubjectModel> options =
                new FirebaseRecyclerOptions.Builder<SubjectModel>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("subDetails").
                                child("BSW").child("Third_Year").child("SubjectList"), SubjectModel.class)
                        .build();

        subjectAdapter = new BSW_ThirdYearSubjectAdapter(options,Role);

        if(Role != null && Role.equals("TeacherStudent")){
            subjectBinding.bswThirdSub.setVisibility(View.GONE);
        }

        DividerItemDecoration dividerItemDecoration = new DividerItemDecoration(getContext(),
                DividerItemDecoration.VERTICAL);
        subjectBinding.bswThirdSubRecycler.addItemDecoration(dividerItemDecoration);
        subjectBinding.bswThirdSubRecycler.setAdapter(subjectAdapter);

        subjectBinding.bswThirdSub.setOnClickListener(new View.OnClickListener() {
            @Override
