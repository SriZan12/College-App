...
import com.example.relianceinternationalcollege.databinding.FragmentBswFirstYearSubjectBinding;
...
import Admin.Courses.SubCourses.BBS.Years.FirstYear.SubjectAdapter;


public class BswFirstYearSubject extends Fragment {

    FragmentBswFirstYearSubjectBinding subjectBinding;
    BswFirstYearSubjectAdapter subjectAdapter;
    String Role;

    public BswFirstYearSubject(String role) {
        this.Role = role;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        subjectBinding =  FragmentBswFirstYearSubjectBinding.inflate(inflater, container, false);

        subjectBinding.bswFirstSubRecycler.setLayoutManager(new LinearLayoutManager(getContext()));
        ...
        FirebaseRecyclerOptions<SubjectModel> options =
                new FirebaseRecyclerOptions.Builder<SubjectModel>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("subDetails").
                                child("BSW").child("First_Year").child("SubjectList"), SubjectModel.class)
                        .build();

        subjectAdapter = new BswFirstYearSubjectAdapter(options,Role);
        DividerItemDecoration dividerItemDecoration = new DividerItemDecoration(getContext(),
                DividerItemDecoration.VERTICAL);
        subjectBinding.bswFirstSubRecycler.addItemDecoration(dividerItemDecoration);
        subjectBinding.bswFirstSubRecycler.setAdapter(subjectAdapter);

        if(Role != null && Role.equals("TeacherStudent")){
            subjectBinding.bswFirstSub.setVisibility(View.GONE);
        }

        subjectBinding.bswFirstSub.setOnClickListener(new View.OnClickListener() {
            @Override
                ...
                Button done = dialog.findViewById(R.id.subjectDone);
                    ...
                    public void onClick(View v) {
                        ...
                        FirebaseDatabase.getInstance().getReference().child("subDetails").
                                child("BSW").child("First_Year").child("SubjectList")
                                .push()
                                .setValue(subDetails)
